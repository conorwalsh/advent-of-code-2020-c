	...
	for (int d = 0; d<count; d++){
		...
		while (all[d][0]!='\n'){
			...
			while(all[d][i]!='\n'){
				...
				if (found==0){
					ucurlet++;
					ulet[ucurlet]=all[d][i];
				}
				// Add all letters to let
				let[curlet]=all[d][i];
				curlet++;
				i++;
			}
		...
		int tot = 0;
		// Check if the number of times a letter occurs is equal to the number of people
		// If so all in group have that letter
		for ( int i = 0; i<=ucurlet; i++){
			int found = 0;
